...
stores = [
    ...
    (
        ...
        "https://github.com/tyj513/audio/raw/main/menu/%E5%8E%9A.jpeg",
    ),
    (
        "第一學生宿舍餐飲中心",
        "小鐵匠廚房",
        ...
        "蔬食/經濟/雞排/豬排/雞腿等飯類",
        ["0-4 11:00-13:00"],
        "https://github.com/tyj513/audio/raw/main/menu/%E5%B0%8F.jpeg",
    ),
    (
        "第一學生宿舍餐飲中心",
        "東大膳綁",
    ...
    (
        ...
        "https://github.com/tyj513/audio/raw/main/menu/al.jpeg",
    ),
    (
        "第二學生宿舍餐飲中心",
        "東大健康茶飲",
    ...
    (
        ...
        True,
        "0937-602-855",
        "海苔飯手卷, 炊飯, 滷肉飯、肉羹飯（湯），飲品、熱壓吐司、車輪餅",
...
def send_reply_based_on_data(data, event):
    if len(data) > 1 and data[-2] == "學":
        ...
        current_time_str = "11:00"  # 假設這是當前時間，你會用實際檢查時間來替換它
        if current_time_str[:2] == "11":  # 檢查是否為特定時間
            response = []
            columns = []
            ...
            for store in stores:
                ...
                if (data[1] == "True" and vegetarian) or (
                    ...
                    if is_open_now(week_schedule):

                        actions = [
                            URIAction(label="撥打電話", uri=f"tel:{phone.strip()}")
                        ]
                        ...
                        column = CarouselColumn(
                            title=f"{store_name} - {'素食' if vegetarian else '葷食'}",
                            text=f"營業中 | {item}\n地點 | {place}",
                            actions=actions,
                        )
